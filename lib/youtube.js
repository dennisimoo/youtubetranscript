...
async function isYouTubeShortEnhanced(videoId, fallbackResult = false) {
  try {
    ...
    // Decrypt cookies from encrypted file
    const crypto = require('crypto');
    let cookiesContent = '';
    const encryptedPath = path.join(process.cwd(), 'cookies.encrypted');
    console.log(`  🔍 Looking for encrypted cookies at: ${encryptedPath}`);
    
    try {
      if (!fs.existsSync(encryptedPath)) {
        throw new Error(`cookies.encrypted not found at ${encryptedPath}`);
      }
      ...
      cookiesContent = decipher.update(encrypted, 'hex', 'utf8') + decipher.final('utf8');
      console.log(`  ✅ Successfully decrypted ${cookiesContent.length} chars`);
    } catch (error) {
      console.log(`  ❌ Failed to decrypt cookies: ${error.message}`);
    }
    ...
    console.log(`  🔍 Last 100 chars: ${cookiesContent.substring(cookiesContent.length - 100)}`);
...
async function getTranscript(videoId) {
  try {
    ...
    // Get the same decrypted cookies content
    const fs = require('fs');
    const path = require('path');
    const crypto = require('crypto');
    let cookiesForTranscript = '';
    try {
      const encryptedPath = path.join(process.cwd(), 'cookies.encrypted');
      console.log(`📍 Looking for cookies at: ${encryptedPath}`);
      if (fs.existsSync(encryptedPath)) {
        const encryptedData = fs.readFileSync(encryptedPath, 'utf8');
        ...
        const iv = Buffer.from(ivHex, 'hex');
        const decipher = crypto.createDecipheriv('aes-256-cbc', key, iv);
        cookiesForTranscript = decipher.update(encrypted, 'hex', 'utf8') + decipher.final('utf8');
        console.log(`✅ Decrypted ${cookiesForTranscript.length} chars for transcript`);
        console.log(`🔍 First 100 chars: ${cookiesForTranscript.substring(0, 100)}`);
      } else {
        console.log(`❌ Cookies file not found at ${encryptedPath}`);
      }
    } catch (error) {
      console.log(`❌ Failed to decrypt cookies for transcript: ${error.message}`);
    }
...
import os
try:
    # Write decrypted cookies to temp file
    cookies_content = '''${cookiesForTranscript.replace(/'/g, "\\'")}'''
