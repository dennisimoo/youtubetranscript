...
async function isYouTubeShortEnhanced(videoId, fallbackResult = false) {
  try {
    ...
    // Build proxy command for yt-dlp using Webshare
    const proxyUrl = `http://${process.env.WEBSHARE_PROXY_USERNAME}:${process.env.WEBSHARE_PROXY_PASSWORD}@${process.env.WEBSHARE_PROXY_HOST}:${process.env.WEBSHARE_PROXY_PORT}`;
    console.log(`  🌐 Using Webshare proxy: ${process.env.WEBSHARE_PROXY_HOST}:${process.env.WEBSHARE_PROXY_PORT}`);
    
    let stdout;
    try {
      // Use yt-dlp to get video metadata with proxy and additional headers
      const result = await execAsync(`yt-dlp --proxy "${proxyUrl}" --cookies cookies.txt --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --add-header "Accept-Language:en-US,en;q=0.9" --sleep-interval 1 --max-sleep-interval 3 -j "${videoUrl}"`);
...
// Simple transcript fetcher using Python API with Webshare proxy
async function getTranscript(videoId) {
  try {
    console.log('🐍 Fetching transcript with Webshare proxy...');
...
import json
try:
    # Initialize YouTubeTranscriptApi with Webshare proxy (only username and password)
    proxy_config = WebshareProxyConfig(
        proxy_username='${process.env.WEBSHARE_PROXY_USERNAME}',
        proxy_password='${process.env.WEBSHARE_PROXY_PASSWORD}'
    )
    
    ytt_api = YouTubeTranscriptApi(proxy_config=proxy_config)
    transcript = ytt_api.get_transcript('${videoId}')
